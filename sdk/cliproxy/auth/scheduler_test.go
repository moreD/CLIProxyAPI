...
func TestQuotaStickyRanking(t *testing.T) {
	...
	knownExhausted := &Auth{ID: "known-exhausted", RuntimeQuota: testQuota(now.Add(time.Hour), 99)}
	if compareQuotaForSticky(unknown, knownExhausted) >= 0 {
		t.Fatalf("unknown quota should rank before exhausted known quota")
	}
	resetOnly := &Auth{ID: "reset-only", RuntimeQuota: &QuotaInfo{Weekly: QuotaWindow{NextFreshAt: now.Add(time.Hour)}}}
	if compareQuotaForSticky(resetOnly, knownExhausted) >= 0 {
		t.Fatalf("reset-only quota should not be treated as known used quota")
	}

	lowUsed := &Auth{ID: "low-used", RuntimeQuota: testQuota(now.Add(time.Hour), 5)}
	highUsed := &Auth{ID: "high-used", RuntimeQuota: testQuota(now.Add(2*time.Hour), 90)}
	if compareQuotaForSticky(lowUsed, highUsed) >= 0 {
		t.Fatalf("lower weekly used percentage should rank first")
	}

	laterReset := &Auth{ID: "a-later", RuntimeQuota: testQuota(now.Add(7*24*time.Hour), 80)}
	earlierReset := &Auth{ID: "b-earlier", RuntimeQuota: testQuota(now, 80)}
	if compareQuotaForStickyAt(earlierReset, laterReset, now) >= 0 {
		t.Fatalf("earlier weekly refresh should increase ranking weight")
	}

	weightedHigherUsage := &Auth{ID: "weighted-higher", RuntimeQuota: testQuota(now, 45)}
	unweightedLowerUsage := &Auth{ID: "unweighted-lower", RuntimeQuota: testQuota(now.Add(7*24*time.Hour), 20)}
	if compareQuotaForStickyAt(weightedHigherUsage, unweightedLowerUsage, now) >= 0 {
		t.Fatalf("refresh weight should normalize weekly used percentage")
	}

	lowerUsed := &Auth{ID: "lower", RuntimeQuota: testQuota(now.Add(time.Hour), 25)}
	higherUsed := &Auth{ID: "higher", RuntimeQuota: testQuota(now.Add(time.Hour), 80)}
	if compareQuotaForSticky(lowerUsed, higherUsed) >= 0 {
		t.Fatalf("lower weekly used percentage should rank first")
	}

	lowFiveHourUsed := &Auth{ID: "low-five-hour", RuntimeQuota: testStickyFiveHourQuota(9, 90)}
	highFiveHourUsed := &Auth{ID: "high-five-hour", RuntimeQuota: testStickyFiveHourQuota(80, 10)}
	if compareQuotaForSticky(highFiveHourUsed, lowFiveHourUsed) >= 0 {
		t.Fatalf("lower weekly used percentage should rank before five-hour used percentage")
	}
	fiveHourOnly := &Auth{ID: "five-hour-only", RuntimeQuota: &QuotaInfo{
		FiveHour: QuotaWindow{UsedPercent: 5, UsedPercentKnown: true},
	}}
	if quotaRankingKnown(fiveHourOnly) {
		t.Fatalf("five-hour-only quota should not be treated as ranking quota")
	}
...
func TestWeeklyQuotaRefreshWeight(t *testing.T) {
	t.Parallel()

	now := time.Date(2026, 5, 17, 12, 0, 0, 0, time.UTC)
	tests := []struct {
		name        string
		nextFreshAt time.Time
		want        float64
	}{
		{name: "missing", want: 1},
		{name: "seven days away", nextFreshAt: now.Add(7 * 24 * time.Hour), want: 1},
		{name: "halfway", nextFreshAt: now.Add(84 * time.Hour), want: 2},
		{name: "refreshing now", nextFreshAt: now, want: 3},
		{name: "past due", nextFreshAt: now.Add(-time.Hour), want: 3},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			if got := weeklyQuotaRefreshWeight(test.nextFreshAt, now); got != test.want {
				t.Fatalf("weeklyQuotaRefreshWeight() = %v, want %v", got, test.want)
			}
		})
	}
}
...
func TestSchedulerPick_StickyRoundRobinUsesQuotaRankingForReplacement(t *testing.T) {
	...
	if got == nil || got.ID != "a" {
		t.Fatalf("pickSingle() startup auth = %v, want a", got)
	}

	scheduler.upsertAuth(&Auth{ID: "a", Provider: "codex", Disabled: true})
	got, errPick = scheduler.pickSingle(context.Background(), "codex", "", cliproxyexecutor.Options{}, nil)
	if errPick != nil {
...
func TestSchedulerPick_StickyRoundRobinSwitchesFromExhaustedWeeklyQuotaToUnknown(t *testing.T) {
	t.Parallel()

	now := time.Date(2026, 5, 17, 12, 0, 0, 0, time.UTC)
	scheduler := newSchedulerForTest(
		&StickyRoundRobinSelector{},
		&Auth{ID: "used", Provider: "codex", RuntimeQuota: testQuota(now.Add(time.Hour), 90)},
	)
	...
	if got == nil || got.ID != "used" {
		t.Fatalf("pickSingle() initial auth = %v, want used", got)
	}

	scheduler.upsertAuth(&Auth{ID: "used", Provider: "codex", RuntimeQuota: testQuota(now.Add(time.Hour), 99)})
	scheduler.upsertAuth(&Auth{ID: "new", Provider: "codex"})

	got, errPick = scheduler.pickSingle(context.Background(), "codex", "", cliproxyexecutor.Options{}, nil)
	if errPick != nil {
		t.Fatalf("pickSingle() after new auth error = %v", errPick)
	}
	if got == nil || got.ID != "new" {
		t.Fatalf("pickSingle() after new auth = %v, want new unknown-quota auth over exhausted weekly usage", got)
	}
}
...
func TestManager_PickNextMixed_StickyRoundRobinUsesQuotaRanking(t *testing.T) {
	...
	for _, auth := range []*Auth{
		{ID: "gemini-a", Provider: "gemini", RuntimeQuota: testQuota(now.Add(time.Hour), 10)},
		{ID: "claude-a", Provider: "claude", RuntimeQuota: testQuota(now.Add(2*time.Hour), 80)},
...
func testQuota(nextFreshAt time.Time, usedPercent float64) *QuotaInfo {
	return &QuotaInfo{
		Weekly: QuotaWindow{
...
func testStickyFiveHourQuota(fiveHourUsedPercent, weeklyUsedPercent float64) *QuotaInfo {
	now := time.Date(2026, 5, 17, 12, 0, 0, 0, time.UTC)
	return &QuotaInfo{
		FiveHour: QuotaWindow{
			UsedPercent:      fiveHourUsedPercent,
			UsedPercentKnown: true,
			NextFreshAt:      now.Add(time.Hour),
			RefreshedAt:      now,
		},
		Weekly: QuotaWindow{
			UsedPercent:      weeklyUsedPercent,
			UsedPercentKnown: true,
			NextFreshAt:      now.Add(7 * 24 * time.Hour),
			RefreshedAt:      now,
		},
	}
}
